...
function removeModule() {
	...
	if (callback && callback.length > 0) {
		if (caller) {
		    try {
		        if (confirm($('mConfirmRemove').innerHTML)) {
                    
                    // Remove the module with AJAX
		            new Ajax.Request(
				        'ParagraphEditModule.aspx?ID=' + parseInt($('ParagraphID').value) + '&action=RemoveModuleSettingsAJAX',
				        {
				            method: 'get',
...
function initForm() {
	...
	$(parent.parent.right.document).fire('shortcut:moduleLoaded');
}

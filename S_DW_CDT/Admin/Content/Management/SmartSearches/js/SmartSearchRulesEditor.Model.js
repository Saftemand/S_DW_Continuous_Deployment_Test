...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.DropDownListCtrl = function (id, val, ds, onChange, config) {
    ...
    if (id != "") {
        ...
        if (config.allowExpression) {
            ...
            addExpression.type = 'button';
            addExpression.innerHTML = '+';
            addExpression.className = 'smartsearch-expression';
...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.LinkSelectorCtrl = function (id, val, onChange) {
    ...
    if (id != "") {
        ...
        input.name = id;
        input.value = val;
        input.setAttribute('style', 'width:200px !important; float: left;');
        ...
        input.onchange = function (e) {
            ...
            document.getElementById(el.id.replace("Link_", "")).value = value;
            if (onChange) onChange(value);
        };
        ...
        a.id = "Href_" + id;
        a.href = "javascript:internal(0, '', '" + id + "', 'on');"
        ...
        var img = document.createElement('img');
        img.id = "ImgBut_" + id;
        img.src = "/Admin/images/Icons/Page_int.gif";
        img.className = "H";
        img.align = "absMiddle";
        img.border = "0";
        ...
        Event.observe(img, 'click', function (e) {
            var el = Event.element(e);
            document.getElementById(el.id.replace("ImgBut_", "Link_")).style.color = '#000000';
        });
...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.EmailLinkCtrl = function (id, val, onChange) {
    ...
    if (id != "") {
        ...
        button.on('click', function() {
		    window.setEmailLink = function(data) {			
		        hiddenValue.value = data.linkId;
//				displayValue.value = JSON.stringify(data);
				storeValue(data);
                displayValue.value = renderValue();
	        };
		    var linkId = hiddenValue.value;
		    if (typeof (value.linkId) != 'unknown' && typeof (value.linkId) != 'undefined')
		        linkId = value.linkId;
            var url = '/Admin/Content/Management/SmartSearches/misc/SelectEmailLink.aspx?callback=opener.setEmailLink'+(linkId ? '&linkId='+linkId : ''),
		    win = window.open(url, '_blank', "displayWindow,width=700,height=400,scrollbars=yes");
        });
...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.PageCtrl = function (id, val, onChange) {
    var result = null,
        ...
        storeValue = function() {
            var value = [ pageUrl.value, pageName.value ].join('||');
            if (onChange) {
                onChange(value);
            }
        };
    ...
    if (id != "") {
        ...
        pageUrl = new Element('input', {
            type: "hidden",
            id: id,
            id: id,
            value: values.pageUrl
        });
        result.appendChild(pageUrl);

        pageName = new Element('input', {
            type: "text",
            maxlength: 255,
            className: "std",
            id: 'Link_'+id,
            ...
            disabled: true,
            smartsearchctrl: 'true'
        });
        // Called from popup window
        pageName.onchange = function() {
            storeValue();
        };

        result.appendChild(pageName);
        ...
        button.on('click', function() {
            var win = window.open('/Admin/Menu.aspx?Action=Internal&Caller='+encodeURIComponent(id),
                "_blank", "resizable=yes,scrollbars=auto,toolbar=no,location=no,directories=no,status=no,width=250,height=375,top=155,left=202");
        });
        result.appendChild(button);
    }
...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.UserCtrl = function (id, val, onChange)
{
    ...
    if(id != "")
    {
        ...
        var button = new Element("img", {
            src: "/Admin/Images/Controls/UserSelector/user.png"
        });
        button.on('click', function ()
        {
...
Dynamicweb.Controls.SmartSearch.SmartSearchRulesEditor.DateTimeCtrl = function (id, val, nameOfMonths, onChange, showTime, showDynamicValues, terminology) {
    ...
    var calendarOnChange = function (e) {
        ...
        var minuteCtrl = document.getElementById(ctrlId + "_Minutes");
        var value = el.value || '';
        if (value.charAt(0) == '@' ||  value.toLowerCase() == "never" || value == "") {
            onChange(value)
            if (timeCtrl && minuteCtrl) {
                timeCtrl.disabled = true;
                minuteCtrl.disabled = true;
            }
        }
        else {
            if (showTime) {
                ...
                var dMinutes = (minuteCtrl == null ? displayedMinute : parseInt(minuteCtrl.value, 10));
    ...
    if (id != "") {
        ...
        result.innerHTML =  '<div id="' + id + '_DateTime" class="date-selector">' +
                            ...
                            '<input type="text" id="' + id + '_DateView" class="date-selector-label"' + (showTime ? ' style="width:151px !important;" ' : ' style="width: 211px !important;"') + ' value="' + displayedDate + '"></input>' +
                            '<a class="date-selector-button mod" title="Select date"></a>' +
                            (showDynamicValues ? '<a class="date-selector-dynamic-values" title="Select date"></a>' : '') +
                            '<div class="date-selector-time" ' + (showTime ? ' ' : 'style="display:none"') + '>' +
                            '<input type="text" id="' + id + '_Hours" name="DateTime$Hour" class="std date-selector-time-hour" value="' + displayedHour + '" ' + (isDynamicValue ? 'disabled' : '') + ' />' +
                            '<div class="date-selector-time-separator">:</div>' +
                            '<input type="text" id="' + id + '_Minutes" name="DateTime$Minute" class="std date-selector-time-minute" value="' + displayedMinute + '" ' + (isDynamicValue ? 'disabled' : '') + ' />' +
        ...
        if (showDynamicValues) {
            ...
            dynamicDateSelector.positionIsSet = false;
            dynamicDateSelector.isShown = false;
            dynamicDateSelector.style.left = '337px';
            dynamicDateSelector.style.zIndex = 4;
            dynamicDateSelector.onmouseleave = function (e) {
                e.target.style.display = "none";
                dynamicDateSelector.isShown = false;
            };

            dynamicDateSelector.onchange = function (e) {
                var timeCtrl = document.getElementById(id + "_Hours");
                var minuteCtrl = document.getElementById(id + "_Minutes");
                ...
                var lbl = p.select('.date-selector-label')[0];
                var hiddenValue = p.select('.date-selector-date')[0];
                ...
                lbl.value = selectedOpt.isCustom ? selectedOptValue : selectedOpt.text;
            ...
            Event.observe(regValueBtn, 'click', function (e) {
                ...
                if (!dynamicDateSelector.positionIsSet) {
                    var p = dynamicDateSelector.parentElement;
                    var lbl = p.select('.date-selector-label')[0];
                    var calBtn = p.select('.date-selector-button')[0];
                    var dynValBtn = p.select('.date-selector-dynamic-values')[0];
                    var wdth = lbl.getWidth() + calBtn.getWidth() + dynValBtn.getWidth();
                    var offSet = lbl.cumulativeOffset();
                    wdth = (wdth -1).toString();
                    wdth += "px !important";
                    dynamicDateSelector.style.cssText += " width: " + wdth + "; left:" + offSet.left + "px;";
                    dynamicDateSelector.positionIsSet = true;
                }
            });
        ...
        if (regDates) {
            for (var i = 0; i < regDates.length; i++) {
                if (regDates[i].value == calendar.value) {
                    date_label.value = regDates[i].key;
                    break;
                }
            }
        }

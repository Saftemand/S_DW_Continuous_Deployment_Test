...
SmartSearch.EditSmartSearch.prototype.initialize = function (id, helpLang, isPreview, isPopup, previewPreffixId, nameId, cacheSettings, calledFrom) {
    ...
    this._cacheSettings = JSON.parse(cacheSettings);
...
SmartSearch.EditSmartSearch.prototype._showPreview = function (isPreview) {
    ...
    var previewLayer = document.getElementById((this._previewPreffixId + "Layer"));
    var previewContent = document.getElementById((this._previewPreffixId + "Content"));
    var previewFrame = document.getElementById((this._previewPreffixId + "Frame"));
    


    var treePreviewState = false;
    if (parent.eCommerce) {
        var ecomTree = parent.eCommerce.SystemTree.getInstance();        
        treePreviewState = (ecomTree.get_SmartSearchPreviewMode() != "");       
    };

    try {
        if (previewLayer && previewContent && previewFrame) {
            buttons.enableButton('cmd_Preview');

            var isCanSetPreview = false;

            if (isPreview || treePreviewState) {
                var isGoogleChrome = (navigator.userAgent.toLowerCase().indexOf('chrome') > -1);
                var formHeight = 640 - (isGoogleChrome ? 0 : 20);

                buttons.activateButton('cmd_Preview');

                previewFrame.src = this.get_preViewUrl();
                var newHeight = 0;

                if (getContentFrame()) {
                    newHeight = getContentFrame().clientHeight - formHeight;                
                }

                if (newHeight > 40) {
                    previewLayer.style.display = "";
                    previewLayer.style.cssText = 'z-index: 500; display:block; position: relative; overflow:hidden; height:' + newHeight + 'px; width:100%; background-color:White; border-top:2px solid #6593CF; border-bottom:1px solid #6593CF;';
                    isCanSetPreview = true;
                }
                else {
                    alert(this.get_terminology()['ResolutionTooLow']);
                }
            }

            if (!isCanSetPreview) {
                buttons.deactivateButton('cmd_Preview');
                previewLayer.style.cssText = 'z-index: 500; display:none; position: relative; overflow:hidden; height:1px; width:100%; background-color:White; border-top:2px solid #6593CF; border-bottom:1px solid #6593CF;';
                previewFrame.src = "";
            }
        }
        else {
            buttons.disableButton('cmd_Preview');
        }
...
SmartSearch.EditSmartSearch.prototype.preview = function () {
    ...
    if (!this.get_previewMode()) {
        ...
        if (parent.eCommerce) {
            var ecomTree = parent.eCommerce.SystemTree.getInstance();        
            ecomTree.set_SmartSearchPreviewMode("");
        };
...
SmartSearch.EditSmartSearch.prototype.get_caller = function () {
    /// <summary>Get caller.</summary>
    return this._calledFrom;
}

SmartSearch.EditSmartSearch.prototype.set_caller = function (caller) {
    /// <summary>Set caller.</summary>
    this._calledFrom = caller;
}
...
SmartSearch.EditSmartSearch.prototype.get_SaveUrl = function (closeWindow) {
    ...
    if (caller && (result.indexOf("calledFrom") < 0)) {
        result += "&calledFrom=" + caller;
    };

    if(this._cacheSettings) {
        result += "&cachingInterval=" + this._cacheSettings.CachingInterval;
    };
...
SmartSearch.EditSmartSearch.prototype.openCacheSettingsDialog = function () {
    var cacheSelector = $('CachingIntervalSelector');
    (this._cacheSettings.CachingInterval != 0 && this._cacheSettings.Enabled) ? cacheSelector.value = this._cacheSettings.CachingInterval : cacheSelector.selectedIndex = 0;
    $('EnableCachinChkbox').checked = this._cacheSettings.Enabled;
    cacheSelector.disabled = this._cacheSettings.Enabled ? "" : "disabled";
    dialog.show('CacheSettingsDlg');
}


SmartSearch.EditSmartSearch.prototype.saveCacheSettings = function () {
    var isCacheEnabled = $('EnableCachinChkbox').checked;
    var cachingSelector = $('CachingIntervalSelector');
    if (isCacheEnabled) {
        this._cacheSettings.CachingInterval = cachingSelector.value;
    } else {
        this._cacheSettings.CachingInterval = 0;
    };
    this._cacheSettings.Enabled = isCacheEnabled;
    dialog.hide('CacheSettingsDlg');
}
...
// Node object
function Node(id, pid, name, url, contextId, title, target, icon, iconOpen, iconClose, open, itemID, hasChildren, disabled, onDoubleClick, dragDrop, additionalCssClass, checkable, checked, additionalAttributes) {
    this.id = id;
    this.pid = pid;
    this.name = name;
    this.url = url;
...
dTree.prototype.add = function (id, pid, name, url, contextId, title, target, icon, iconOpen, iconClose, open, itemID, hasChildren, disabled, onDoubleClick, dragDrop, additionalCssClass, checkable, checked, additionalAttributes) {
    if (this.config.autoID && this.aNodes.length > 0) {
        id = this.aNodes[this.aNodes.length - 1].id + 1;
    }
    var newNode = new Node(id, pid, name, url, contextId, title, target, icon, iconOpen, iconClose, open, itemID, hasChildren, disabled, onDoubleClick, dragDrop, additionalCssClass, checkable, checked, additionalAttributes);
...
dTree.prototype.node = function (node, nodeId) {
    ...
    var str = '<div class="dTreeNode" id="dtn' + nodeId + '" style="display:' + ((!this.config.showRoot && this.root.id == node.pid) ? 'none' : '') + '" ' + additionalAttribuesString + ' >';
    ...
    if (node.checkable) {
        ...
        str += '<input id="tchk' + node.id + '" type="checkbox" ' + checked + clientCheck + ' data-node-id="' + node.id + '"  />';
    }

    if (this.config.useIcons) {
        if (!node.icon) node.icon = (this.root.id == node.pid) ? this.icon.root : ((node._hc) ? this.icon.folder : this.icon.node);
        if (!node.iconOpen) node.iconOpen = (node._hc) ? this.icon.folderOpen : this.icon.node;
        if (!node.iconClose) node.iconClose = (node._hc) ? this.icon.folder : this.icon.node;
    ...
    if (node.url || (node.onDoubleClick && node.onDoubleClick.length > 0)) {
        str += '<a id="s' + this.obj + nodeId + '" nodeID="' + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '"'
...
dTree.prototype.indent = function (node, nodeId) {
    ...
    if (this.root.id != node.pid) {
        ...
        if (node._hc) {
            str += '<a href="javascript: ' + onExpand + '"><img id="j' + this.obj + nodeId + '" src="';
            ...
            str += '<img id="f' + this.obj + nodeId + '" src="' + img + '" alt="" />';
        }
...
dTree.prototype.ajax_addNodes = function (nodes) {
    ...
    for (var i = 0; i < nodes.items.length; i++) {
        ...
        var newNode = new Node(item.id, item.pid, item.name, item.url, item.contextId, item.title,
            item.target, item.icon, item.iconOpen, item.iconClose, false, item.itemID, item.hasChildren == 'true', item.disabled == 'true', item.onDoubleClick, item.dragDrop, item.additionalCssClass, item.checkable, item.checked, item.additionalAttributes);

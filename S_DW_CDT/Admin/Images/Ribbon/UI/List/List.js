...
var List = {
    ...
    /*sortAscending: function(url) {
    url = url.replace(/\[0]/g, encodeURIComponent(ContextMenu.callingID));
    url = url.replace(/\[1]/g, 'ASC');
    location = url;
    },
    sortDescending: function(url) {
    url = url.replace(/\[0]/g, encodeURIComponent(ContextMenu.callingID));
    url = url.replace(/\[1]/g, 'DESC');
    location = url;
    },
    filter: function() {
    dialog.show('__ListFilterDlg');
    },
    applyFilter: function(url) {
    dialog.hide('__ListFilterDlg');
    url = url.replace(/\[0]/g, encodeURIComponent(ContextMenu.callingID));
    url = url.replace(/\[1]/g, encodeURIComponent(document.getElementById('__listFilterString').value));
    location = url;
    },*/
    ...
    columnsDialog: function (controlId) {
        ...
        if (List.dialogSelectedControlId == '' && List.dialogSelectedColumnId == '') {
            ...
            if (columns && columns.length > 0) {
                ...
                span.addClassName("list-dialog-columns-item-selected");
            }
        }
    },
    ...
    dialogSelectColumn: function (controlId, columnId) {
        ...
        if (old) {
            old.removeClassName('list-dialog-columns-item-selected');
        }
    ...
    expand: function (controlID, rowID) {
    ...
    CollapseFilters: function () {
        ...
        if ($('filtersDiv').style.display == 'none') {
            ...
            setTimeout("$('FilterCollapseImage').src = '/Admin/Images/Ribbon/UI/List/Collapse.png'", time * 1000);
            setTimeout("$('FilterCollapseImage').title = $('CollapseText').value;", time * 1000);
            $('IsCollapsed').value = 'False';
        } else {
            // Collapse
            Effect.BlindUp('filtersDiv', { duration: time });
            setTimeout("$('FilterCollapseImage').src = '/Admin/Images/Ribbon/UI/List/Uncollapse.png';", time * 1000);
...
RowsLoader.toggle = function (rowID) {
    ...
    if (childsRow) {
        if (img)
            img.writeAttribute('src', (childsRow.visible() ? '/Admin/Images/Ribbon/UI/Tree/img/nolines_plus.gif' :
                '/Admin/Images/Ribbon/UI/Tree/img/nolines_minus.gif'));

...
ContextMenuViewTrigger.prototype.enableViewByButtonsIDs = function(buttons) {
    ...
    if (buttons && buttons.length > 0) {
        ...
        menuItems = $$('div[id="' + this.get_menuID() +
        ...
        if (menuItems && menuItems.length > 0) {
            for (var i = 0; i < menuItems.length; i++) {
                ...
                /* Showing button */
                if ((this._findIndex(buttons, item.id) >= 0 || itemIsDivider)) {
        ...
        if (item.isDivider) {
            if (i == visibleItems.length - 1)
                ...
                if (i >= visibleItems.length) {
                    visibleItems[startingDividerIndex].element.hide();
                }
            }
        }
    }

...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<%= Base.GetCulture() %>">
	<head runat="server">
		<title><%=Translate.Translate("Maps", 9)%></title>
<style type="text/css">
html, body {
	height: 100%;
	margin: 0;
...
#infoWindowContent .latlng {
	...
	<div id="info">
		...
		<button type="button" id="btnSaveLocation" style="visibility: hidden"><dw:TranslateLabel runat="server" Text="Save location"/></button>
	</div>
		<div id="infoWindowContent" style="display: none">
			...
			<% End If %>
			<div class="latlng">
				<dw:TranslateLabel runat="server" Text="Location" />: (<span id="lat">0</span>, <span id="lng">0</span>)
			</div>
		</div>
		...
		var formatDouble = function(value) {
			var numberFormat = {
				numberGroupSeparator: '<%= Base.JSEnable(Base.GetCulture().NumberFormat.NumberGroupSeparator) %>',
				numberDecimalSeparator: '<%= Base.JSEnable(Base.GetCulture().NumberFormat.NumberDecimalSeparator) %>'
			},
		...
		showLocation = function(location) {
			document.getElementById('lat').innerHTML = formatDouble(location.lat());
			document.getElementById('lng').innerHTML = formatDouble(location.lng());
		}

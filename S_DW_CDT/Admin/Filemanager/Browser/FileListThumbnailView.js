...
ThumbnailView.prototype.selectAllFiles = function (selected) {
    var rows = $$('tbody[id="Files_body_stretch"] > tr.listRow > td > div.fm_column_thumbnail');
    if (rows) {
...
ThumbnailView.prototype.rowIsSelected = function (rowID) {
    if (rowID.indexOf('row') < 0)
        rowID = 'row' + rowID;
    var row = $$('tbody[id="Files_body_stretch"] > tr.listRow > td > div.fm_column_thumbnail[id="' + rowID + '"]');
    var ret = false;
    var checkbox = null;
    if (row) {

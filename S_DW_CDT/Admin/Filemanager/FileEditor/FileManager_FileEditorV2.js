...
Page.onLoad = function () {
    ...
    /* attaching onclick event to the 'View tags' button */
    createEvent('cmdToggleTemplateTags', 'click',
		function (e) { TemplateTagsDialog.toggle(e); });

    /* Template tags slider events */
    createEvent('rowSlider', 'mousedown', function (e) { TemplateTagsSlider.attach(e); });
    createEvent('rowSlider', 'selectstart', function (e) { if (e) e.preventDefault(); return false; }); 
    createEvent(document, 'mousemove', function (e) { TemplateTagsSlider.slide(e); });
    createEvent(document, 'mouseup', function (e) { TemplateTagsSlider.detach(e); });
    ...
    /* Initializing modal windows */

    Editor.ModalWindow.register('SaveAsDialog', {
        onInitialize: SaveAsDialog.initialize,
        onOK: SaveAsDialog.ok,
        onCancel: SaveAsDialog.cancel,
        width: 400,
        height: 75,
        title: document.getElementById('mSaveAsDialogTitle').innerHTML,
        objectLink: { parent: 'SaveAsDialog', child: 'SaveAsDialog' }
    });

    Editor.ModalWindow.register('XsltDialog', {
        onInitialize: XsltDialog.initialize,
        onFinalize: XsltDialog.finalize,
        onOK: XsltDialog.ok,
        onCancel: XsltDialog.cancel,
        width: 400,
        height: 225,
        title: document.getElementById('mXsltDialogTitle').innerHTML
    });

    Editor.ModalWindow.register('FindDialog');
    Editor.ModalWindow.initialize(Page.PopUpInstance);
    
    var callerCallback = Page.getCallerCallbackFunc();

    if (callerCallback) {
        Page.PopUpInstance.add_ok(callerCallback);
    }
}
...
Page.getCallerCallbackFunc = function() {
    var result = window.opener,
        path,
        funcName = $("WindowCallerCallbackFunc").value;

    if (funcName) {
        path = funcName.split('.');
        if (path) {
            path.each(function (el) {
                var clean = el.replace('()', '');
...
Editor.save = function (withoutXsltCheck, onSaved) {
    ...
    if (this.isNewFile() || fileConvertedXslt) {
        ...
        if (typeof (onSaved) === 'function' && typeof (callerCallback) === 'function') {
        ...
        this.doSave(callback, false, withoutXsltCheck);
    }
}
...
Editor.tryGetCurrentFile = function() {
    var ret = '';
    var ctrl = null;
    var probe = ['input.saveas-filename', '#XsltFilename'];
    
    for(var i = 0; i < probe.length; i++) {
        ctrl = Editor.ModalWindow.find(probe[i]);
        if(ctrl && ctrl.value) {
            ret = ctrl.value;
            break;
        }
...
Editor.saveAndClose = function() {
    ...
    if (this.isNewFile() || fileConvertedXslt) {
        ...
        if (typeof (callerCallback) === 'function') {
...
Editor.doSave = function(onSave, fromSaveAs, withoutXsltCheck) {
    ...
    var reloadCaller = $('WindowCallerReload').value.toLowerCase() === 'false' ? false : true;
    ...
    if (fromSaveAs != null && fromSaveAs) {
        fName = Editor.ModalWindow.find('input.saveas-filename').value;
        fDir = SaveAsDialog.getDirectory()[0];

        /* Data comes from the XsltDialog */
        if (typeof (fName) == 'undefined') {
            fName = Editor.ModalWindow.find('#XsltFilename').value;
            fDir = Editor.ModalWindow.find('#XsltDirectory').value;
        }
    }
        ...
        if (ext.length > 0 && (ext == 'xsl' || ext == 'xslt')) {
            this.checkXslt(function(data) {
                if (data.length > 0) {
                    ...
                    Editor.sendFile(fName, fDir, onSave, reloadCaller);
            });
        }
        else
            Editor.sendFile(fName, fDir, onSave, reloadCaller);
...
Editor.sendFile = function (fileName, fileDir, onFileSent, reloadCaller) {
    ...
    if (reloadCaller === null || typeof (reloadCaller) === 'undefined') {
        reloadCaller = true;
    }
...
Editor.translateTemplate = function () {
    var file = $("InitialFileName").value;
	var folder = $("InitialFileDir").value;
	var caller = $('WindowCaller');
	var wnd = null;
		
	if(!$('cmdTranslateTemplate').hasClassName('disabled')) {
	    if(caller.value == 'translate' && window.opener && !window.opener.closed) 
	        wnd = window.opener;
	    else {
	        caller.value = '';
	        wnd = window.open('/Admin/FileManager/Translation/Translation_TranslateTemplate.aspx?File=' + (folder + '/' + file) + '&Caller=edit', 'dwEditTranslateWnd',
...
var TemplateTagsSlider = new Object();

TemplateTagsSlider.attach = function(e) {
    var evt = e ? e : window.event;
    __slider_current = evt.clientY;
}

TemplateTagsSlider.detach = function() {
    __slider_current = null;
}

TemplateTagsSlider.slide = function(e) {
    var evt = e ? e : window.event;
    var delta = 0;

    var topTree = $('divGeneralTagsTree');
    var bottomTree = $('divTagsTree');

    var newHeight = topTree.getHeight();
    var moduleTagsHeight = bottomTree.getHeight();
    var bottomTreeTop = parseInt(bottomTree.getStyle('top').replace(/px/gi, ''));

    if (TemplateTagsSlider.isAttached()) {
        delta = evt.clientY - __slider_current;

        if ((delta < 0 && newHeight > 50) || (delta > 0 && moduleTagsHeight > 50)) {
            topTree.setStyle({ height: (newHeight + delta) + 'px' });
            bottomTree.setStyle({ top: bottomTreeTop + delta + 'px' });
            __slider_current = evt.clientY;
        } else {
            TemplateTagsSlider.detach();
        }
    }

}

TemplateTagsSlider.isAttached = function() {
    return __slider_current != null;
}

TemplateTagsSlider.reset = function() {
    var topTree = $('divGeneralTagsTree');
    var bottomTree = $('divTagsTree');

    topTree.setStyle({ height: '160px' });
    bottomTree.setStyle({ top: '354px' });
}
...
// becomes @if (Dynamicweb.Input.FormatBoolean(ol.GetValue("Ecom:Order:OrderLine.ProductVariantText"))){<text><b>AreaID defined.<br /></b></text>}
TemplateRazor.loopIfDefinedReplacer = function (loopItem, match, ifName, ifContent, offset, s) {
    ...
    return '@if(Dynamicweb.Input.FormatBoolean(' + loopItem + '.GetValue("' + ifName + '"))){<text>' + ifContent + '</text>}';
}
...
// becomes @if(Dynamicweb.Input.FormatBoolean(GetValue("DwAreaID"))){<text><b>AreaID defined.<br /></b></text>}
TemplateRazor.ifDefinedReplacer = function (match, ifName, ifContent, offset, s) {
    ...
    return '@if(Dynamicweb.Input.FormatBoolean(GetValue("' + ifName + '"))){<text>' + ifContent + '</text>}';
}

...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        function validateFields() {
            ...
            if (recipientList != null && recipientList.length > 0) {
                ...
                for (var i = 0; i < recipentEmails.length; i++) {
                    if (!recipentEmails[i].match(pattern)) {
                        alert('<%= Backend.Translate.JsTranslate("The recipients email has incorrect format. Please specify correct recipient email address.") %>');
                        return false;
                    }
                }
            }
            ...
            if (recipientList != null && recipientList.length <= 0) {
                alert('<%= Backend.Translate.JsTranslate("Please specify the recipient email address.") %>');
                return false;
            } else if (!$('txSenderName').value) {
                alert('<%= Backend.Translate.JsTranslate("Please specify the sender name.") %>');
                $('txSenderName').focus();
                return false;
            } else if (!$('txSenderEmail').value) {
                alert('<%= Backend.Translate.JsTranslate("Please specify the sender email.") %>');
                $('txSenderEmail').focus();
                return false;
            } else if (!($('txSenderEmail').value).match(pattern)) {
                alert('<%= Backend.Translate.JsTranslate("The sender email has incorrect format. Please specify correct sender email address.") %>');
                $('txSenderEmail').focus();
                return false;
            } else if (!$('txSubject').value) {
                alert('<%= Backend.Translate.JsTranslate("Please specify the email subject.") %>');
                $('txSubject').focus();
                return false;
            } else {
                var confirmText = '<%= Backend.Translate.JsTranslate("The sending of leads email will be scheduled for ")%>' + getScheduledDate() + '<%=Backend.Translate.JsTranslate(". Do you want to continue?")%>';
        ...
        <dw:GroupBox ID="gbScheduling" Title="Scheduling" runat="server">
            <table>
                <tr>
                    ...
                    <td colspan="2" >
                        <dw:DateSelector ID="dsScheduleDate" runat="server" IncludeTime="True" ShowAsLabel="False" />
                    </td>
                </tr>
                <tr>
                    <td style="width: 170px">

...
function ChangeBase(base) {
    ...
    switch (base) {
        ...
        case 1:
            ...
            $("conds").style.display = "inline";
            tableTR.style.display = "inline";
...
function showHelper( obj ) {
    ...
    if (oper == "IN" || oper == "NOT IN" || oper == "BETWEEN") {
        ...
        //url = '../QueryHelper.aspx?ref=' + [rowId, oper, field] + "&value=" + value + "&usePlings=" + usePlings;
    }else{
        showRequestHelper(rowId, value, false);
        //url = '../RequestHelper.aspx?id=' + rowId + "&value=" + value;
    }
}
...
function setContentHeight(heightReduction) {
    ...
    var newHeight = parent.getContentFrameHeight() - 1 - $("Ribbon").getHeight() - heightReduction;
    if (newHeight < 0) {
        return false;
    }else{
...
function Previewer() {
    ...
    if ($('PreviewLayer').getStyle('display') == 'none') {
        //var contentHeight = $("content").getHeight() - 400 - 3 + 'px'; //Preview window height is 400px and 3px borders
        if (!setContentHeight(403)) {
            alert(txtResolutionTooLow);
            setContentHeight(0);

...
GroupTree.contextMenu.addSmartSearch = function (addToRoot) {
    GroupTree.setContent('/Admin/Content/Management/SmartSearches/EditSmartSearch.aspx', '?CMD=ADD_SMART_SEARCH' + '&calledFrom=userMgr' + GroupTree.userSmartSearchPreviewMode + '&providerType=Dynamicweb.Modules.Searching.SmartSearch.UserProviderSmartSearch');
}
...
GroupTree.sortNodes = function (a, b) {
    ...
    if (a == b) result = 0;
        ...
        if (node1.itemID == "SMART_SEARCH" || node2.itemID == "SMART_SEARCH") {
...
function setContentIsVisible(isVisible) {
    ...
    $("cellContentLoading").setStyle({ display: (isVisible ? 'none' : ''), left: '201px' });
}

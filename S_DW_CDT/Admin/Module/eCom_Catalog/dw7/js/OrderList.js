...
window.onload = function () {
    ...
    for (var i = 0; i < forms.length; i++) { forms[i].onkeypress = function () { if (event.keyCode == 13) this.submit() } };
}
...
function viewOrder(orderID, fullEdit) {
    //location = '/Admin/Module/eCom_Catalog/dw7/Edit/EcomOrder_Edit.aspx?ID=' + orderID;
    if (!!fullEdit) {
	...
	var orderType = $F("isQuotes") == "True" ? "&OrderType=quotes" : "";
...
function reloadPrintPreview(template) {
	...
	var url = '/Admin/Module/eCom_Catalog/dw7/PrintOrders.aspx' +
			  '?' + urlParameters +
			  '&template=eCom7/Order/' + currentTemplate +
...
function setDatePreset() {
	...
	var preset = document.getElementById('List:DatePresetFilter').value;
	...
	// Reset selection
	if (preset != '')
		document.getElementById('List:DatePresetFilter').selectedIndex = 0;

...
eCommerce.MasterPage.prototype.stretchTreeContent = function() {
    ...
    if (containerHeight > 0) {
        this._cache.tree.style.height = containerHeight + 'px';
...
eCommerce.SystemTree.prototype.get_SmartSearchPreviewMode = function () {
    return this._smartSearchPreviewMode;
}

eCommerce.SystemTree.prototype.set_SmartSearchPreviewMode = function (previewMode) {
    this._smartSearchPreviewMode = previewMode;
}
...
eCommerce.SystemTree.prototype.nodeClick = function(nodePath, nodeID, action, resetTreeState) {
    ...
    if (typeof (action) == 'function') {
        ...
        if (action.indexOf('javascript:') == 0) {
            ...
            action += this.get_SmartSearchPreviewMode();
...
eCommerce.editGroup = function() {
    ...
    var url = "/admin/module/ecom_catalog/dw7/Edit/EcomGroup_Edit.aspx?ID=" + eCommerce.callingGroupID() + eCommerce.GetSmartSearchPreviewMode();
    document.location.href = url;
}
...
eCommerce.startPermissions = function(nodeName, page) {
    ...
    var returnvalue = window.open(page, "Wizard", "displayWindow, left=" + left + ", top=" + top + ", screenX=" + left + ", screenY=" + top + ", width=" + width + ", height=" + height + ", scrollbars=no");
}
...
eCommerce.searchesMenuOnShow = function () {
    var tree = eCommerce.SystemTree.getInstance();
    var nodeItemID = tree.innerTree.selectedNode;

    tree.innerTree.s(ContextMenu.callingID);

    if (nodeItemID != ContextMenu.callingID) {
        var id = ContextMenu.callingItemID;

        eCommerce.updateTreeAfterContextMenu();

        if (id && id != '') {
            id = id.replace('/GROUPS/SMART_SEARCH/', '');
            location.href = '/Admin/module/eCom_Catalog/dw7/AllProducts.aspx?SmartSearchID=' + id + tree.get_SmartSearchPreviewMode();
        }
    }
}

eCommerce.AddSmartSearch = function () {
    var smartSearchPreviewMode = eCommerce.SystemTree.getInstance().get_SmartSearchPreviewMode() || eCommerce.GetSmartSearchPreviewMode();
    location.href = '/Admin/module/eCom_Catalog/dw7/EditProductSmartSearch.aspx?CMD=ADD_SMART_SEARCH' + smartSearchPreviewMode + '&callerID=ecom';
}
...
eCommerce.OnAssortmentShopSelected = function (inst)
{
    ...
    for(i = 0; i < checkedNodes.length; i++)
    {
        ...
        if(!exists)
        {
            ...
            html += "<td align=right><a href=javascript:eCommerce.DetachShop('" + id + "');><img src=../images/delete_small.gif align=absmiddle title='" + delTitle + "' name=delDWRow border=0></a></td></tr>";
...
eCommerce.OnAssortmentGroupSelected = function(inst, groupid, groupname)
{
    ...
    if(inst == null)
    {
        ...
        if(!exists)
        {
            ...
            html += "<td align=right><a href=javascript:eCommerce.DetachGroup('" + groupid + "');><img src=../images/delete_small.gif align=absmiddle title='" + delTitle + "' name=delDWRow border=0></a></td></tr>";
    ...
    else
    {
        ...
        for(i = 0; i < checkedNodes.length; i++)
        {
            ...
            if(!exists)
            {
                ...
                html += "<td align=right><a href=javascript:eCommerce.DetachGroup('" + id + "');><img src=../images/delete_small.gif align=absmiddle title='" + delTitle + "' name=delDWRow border=0></a></td></tr>";
...
eCommerce.OnAssortmentProdSelected = function (inst, arrStr)
{
    ...
    if(inst == null)
    {
        var arrProducts = arrStr.split(',');

        for(i = 0; i < arrProducts.length; i++)
        {
            var productProps = arrProducts[i].split(':');
            var product = new Object();

            product.id = productProps[0];
            product.varId = productProps[1];
            product.name = productProps[2];
            items[i] = product;
        }
    }
    else
    ...
    for(i = 0; i < items.length; i++)
    {
        ...
        if(!exists)
        {
            ...
            html += "<td align=right><a href=javascript:eCommerce.DetachProduct('" + prodKey + "');><img src=../images/delete_small.gif align=absmiddle title='" + delTitle + "' name=delDWRow border=0></a></td></tr>";
...
eCommerce.EditSmartSearch = function (id, backUrl) {
    if (!id) {
        id = ContextMenu.callingItemID;
    };
    if (id && id != '') {
        id = id.replace('/GROUPS/SMART_SEARCH/', '');
        location.href = '/Admin/module/eCom_Catalog/dw7/EditProductSmartSearch.aspx?ID=' + id + '&CMD=EDIT_SMART_SEARCH' + eCommerce.GetSmartSearchPreviewMode() + '&callerID=ecom' + '&backurl=' + (backUrl || "");
    }
}

eCommerce.DeleteSmartSearch = function () {
    var id = ContextMenu.callingItemID;
    if (id && id != '') {
        eCommerce.updateTreeAfterContextMenu();
        id = id.replace('/GROUPS/SMART_SEARCH/', '');
        location.href = '/Admin/module/eCom_Catalog/dw7/EditProductSmartSearch.aspx?ID=' + id + '&CMD=DELETE_SMART_SEARCH&callerID=ecom';
    }
}

eCommerce.GetSmartSearchPreviewMode = function () {    
    return document.location.href.indexOf("preview") == -1 ? "" : (document.location.href.indexOf("preview=on") == -1 ? "&preview=off" : "&preview=on");
}
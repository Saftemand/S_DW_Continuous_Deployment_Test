...
Optimize.writeToEditForm = function(params) {
    ...
    if (params && parent) {
        ...
        if (typeof (parent.Dynamicweb) != 'undefined') {
            wnd = parent.Dynamicweb.Controls.PopUpWindow.current(window);
            if (wnd) {
                wnd.hide();
            }
        }
...
Optimize.PhraseList.chooseProduct = function() {
    ...
    window.open('/Admin/Module/eCom_Catalog/dw7/edit/EcomGroupTree.aspx?CMD=ShowProd&AddCaller=1&caller=' + caller, '', 
...
Optimize.PhraseList.setTitle = function() {
    /// <summary>Sets the title of the hosting PopUpWindow control.</summary>
    ...
    if (msg) {
        ...
        if (parent && typeof (parent.Dynamicweb) != 'undefined') {
            wnd = parent.Dynamicweb.Controls.PopUpWindow.current(window);
            if (wnd) {
                wnd.set_title(text);
            }
...
Optimize.PhraseList.watchProductChange = function() {
    /// <summary>Tracks the change of the hidden field representing a product that has been choosen by a user.</summary>

    var field = document.getElementById('ID_CustomProductID');

    if (field.value.length > 0) {
        location.href = 'Default.aspx?ProductID=' + field.value + '&t=' + (new Date()).getTime();
    } else {
        setTimeout(Optimize.PhraseList.watchProductChange, 500);
    }
}
...
Optimize.OptimizeResults.cancel = function() {
    ...
    if (canQuit) {
        if (parent != null && typeof (parent.Dynamicweb) != 'undefined') {
            wnd = parent.Dynamicweb.Controls.PopUpWindow.current(window);
            if (wnd) {
                wnd.hide();
            }
...
Optimize.OptimizeResults.updateResult = function(result, field) {
    ...
    if (field && result && Optimize.OptimizeResults.isActiveField(field)) {
        if (result.status == Optimize.OptimizeResults.OptimizeFieldValidityState.valid) {
            img = '/Admin/Images/Ribbon/Icons/check.png';
        } else if (result.status == Optimize.OptimizeResults.OptimizeFieldValidityState.partiallyValid) {
            img = '/Admin/Images/Ribbon/Icons/warning.png';
        } else {
            img = '/Admin/Images/Ribbon/Icons/error.png';
        }
...
Optimize.OptimizeResults.OptimizeField.prototype.updateResult = function(result) {
    ...
    if (result) {
        if (result.status == Optimize.OptimizeResults.OptimizeFieldValidityState.valid) {
            img = '/Admin/Images/Ribbon/Icons/Small/check.png';
        } else if (result.status == Optimize.OptimizeResults.OptimizeFieldValidityState.partiallyValid) {
            img = '/Admin/Images/Ribbon/Icons/Small/warning.png';
        } else {
            img = '/Admin/Images/Ribbon/Icons/Small/error.png';
        }

        document.getElementById(this.get_name() + '_ResultMark').src = img;
    }

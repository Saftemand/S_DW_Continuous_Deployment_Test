...
Dynamicweb.Items.ItemTypeEdit.reloadPageTree = function (close, destination, msg) {
    var src, openTo = "ContentTypes";
    if (msg) alert(msg);
    ...
    if (params.OpenTo) {
        src = src.replace("OpenTo=" + encodeURIComponent(params.OpenTo), "OpenTo=" + encodeURIComponent(openTo));
        src = src.replace("OpenTo=" + params.OpenTo, "OpenTo=" + encodeURIComponent(openTo));
    } else {
        src = src + "?OpenTo=" + encodeURIComponent(openTo);
    }
...
Dynamicweb.Items.ItemTypeEdit.prototype.get_optionsPopup = function () {
    return this._optionsPopup;
};

Dynamicweb.Items.ItemTypeEdit.prototype.set_optionsPopup = function (value) {
    this._optionsPopup = value;
};
...
Dynamicweb.Items.ItemTypeEdit.prototype.show_fileManager = function (caller) {
    var self = this, fileManager;

    fileManager = window.open(encodeURI('/Admin/FileManager/Browser/Default.aspx?Mode=browselinkcallback&CloseOnSelect=True&FileManagerAllowedExtensions=.gif,.jpg,.jpeg,.png', "DW_browse_window", "resizable=yes,scrollbars=auto,toolbar=no,location=no,directories=no,status=no,minimize=no,width=800,height=600,left=200,top=120"));

    if (fileManager) {
        fileManager[fileManager.addEventListener ? 'addEventListener' : 'attachEvent']((fileManager.attachEvent ? 'on' : '') + 'load', function () {
            window.document.fire('fileManager:open', { caller: caller });
            fileManager.onbeforeunload = function () {
                window.document.fire('fileManager:close', { caller: caller });
            };

            fileManager.RunCallbackFunction = function (strFile, strFile) {
                window.document.fire('fileManager:select', { file: strFile, caller: caller });
            };
        }, false);
    }
};
...
Dynamicweb.Items.ItemTypeEdit.prototype.initialize = function () {
    var self = this,
        ...
        areaRules,
        editorTypesJSON,
        toggleModuleRuleCheckbox = function (show) {
            if (show) {
        ...
        togglePageViewSelect = function (show) {
            if (show) {
                structureTypeRulePage.setStyle({ marginTop: '5px', marginBottom: '5px' })
                pageViewSelectContainer.show();
            } else {
                pageViewSelectContainer.hide();
                structureTypeRulePage.setStyle({ marginTop: '0px', marginBottom: '0px' })
            }
        };
    ...
    structureTypeRulePage.setStyle({ whiteSpace: 'unset' });
    ...
    structureTypeRulePage.appendChild(pageViewSelectContainer);
    ...
    moduleRule.removeChild(moduleRuleInput);
    moduleRule.removeChild(moduleRuleLabel);

    moduleRuleContainer = document.createElement('div');
    moduleRuleContainer.appendChild(moduleRuleInput);
    moduleRuleContainer.appendChild(moduleRuleLabel);
    structureTypeRuleParagraph.appendChild(moduleRuleContainer);
    ...
    if (!this._isCodeFirst) {
        ...
        $('edit-options-activator').observe('click', function () {
            var popup = self.get_optionsPopup();
        ...
        var expr = $$('ul.regex-library li');
        for (var i = 0; i < expr.length; i++) {
            $(expr[i]).observe('click', function (e) {
                var rx = '';
                var elm = Event.element(e);
                var field = $('param-validationexpression');
                var tagName = (elm.tagName || elm.nodeName).toLowerCase();
                ...
                if (elm) {
                    rx = elm.readAttribute('data-regex');
        ...
        RichSelect.onItemSelect = Dynamicweb.Items.ItemTypeEdit.onIconChanged;

        window.document.on('fileManager:select', function (event) {
            var query, size, list, li, updateList;

            if (event.memo.caller) {
                size = event.memo.caller.readAttribute('data-size');

                //oops, it's not our event
                if (!size) {
                    return;
                }

                size = size.toLowerCase().strip();

                li = $$('li.richselectitem[data-custom-path="' + event.memo.file + '"][data-size="' + size + '"]')[0];

                if (li) {
                    RichSelect.setselected(li.down(), 'LargeIcon')

                    if (size === 'small') {
                        RichSelect.setselected(li.down(), 'SmallIcon')
                    }

                    return;
                }

                new Ajax.Request('/Admin/Content/Items/ItemTypes/ItemTypeEdit.aspx', {
                    parameters: {
                        AJAX: 'IconValidation',
                        CustomIconName: event.memo.file,
                        CustomIconSize: size
                    },
                    onCreate: function () {
                    ...
                    onSuccess: function (transport) {
                        var obj, errorMsg = '', largeIconsList, smallIconsList;
                        ...
                        if (transport.responseText.isJSON()) {
                            ...
                            if (obj.Error) {
                                ...
                                if (obj.Name && obj.LargeIcon && obj.SmallIcon) {
                                    largeIconsList = $('LargeIcon_selectitems');
                                    largeIconsList.innerHTML += obj.LargeIcon;
                                    smallIconsList = $('SmallIcon_selectitems');
                                    smallIconsList.innerHTML += obj.SmallIcon;

                                    li = $('LargeIcon' + obj.Name);
                                    RichSelect.setselected(li.down(), 'LargeIcon')

                                    if (size === 'small') {
                                        li = $('SmallIcon' + obj.Name);
                                        RichSelect.setselected(li.down(), 'SmallIcon')
                                    }
                                } else {
                    ...
                    onComplete: function () {
                        Dynamicweb.Items.ItemTypeEdit.hide_overlay(30);
                    }
                });
            }
        });
...
Dynamicweb.Items.ItemTypeEdit.onRichSelectChanged = function (value, e, controlId) {
    if (controlId === 'LargeIcon' || controlId === 'SmallIcon') {
        Dynamicweb.Items.ItemTypeEdit.onIconChanged(value, e, controlId);
    }
    else if (controlId === 'Item_type') {
        var sortBy = $('Sort_by');
        if (sortBy) {
...
Dynamicweb.Items.ItemTypeEdit.onIconChanged = function (value, e, controlId) {
    var icon, type;

    if (controlId === 'LargeIcon' || controlId === 'SmallIcon') {
        icon = $('LargeIcon' + value);
        if (icon) {
            type = icon.readAttribute('data-type');

            if (type === 'custom') {
                $('CustomLargeIcon').value = icon.readAttribute('data-custom-path')
            } else {
                $('CustomLargeIcon').value = '';
            }

            if (controlId == 'SmallIcon') {
                RichSelect.setselected(icon.down(), 'LargeIcon');

                icon = $('SmallIcon' + value);
                type = icon.readAttribute('data-type');

                if (type === 'custom') {
                    $('CustomSmallIcon').value = icon.readAttribute('data-custom-path')
                } else {
                    $('CustomSmallIcon').value = '';
                }
            }
        }
    }
...
Dynamicweb.Items.ItemTypeEdit.prototype._onFieldTypeChanged = function (isInherited) {
    ...
    $('insert-regex-activator').writeAttribute('class', !allowChooseExpression ? 'activator-disabled' : '');
    $('insert-regex-image').writeAttribute({
        'src': '/Admin/Images/Ribbon/Icons/Small/' + (allowChooseExpression ? 'scroll2' : 'scroll2grey') + '.png',
        'class': !allowChooseExpression ? 'activator-disabled' : ''
    });
...
Dynamicweb.Items.ItemTypeEdit.prototype.initializeAdvancedSettings = function (settings) {
    ...
    var inheritanceInfo = dialog.select('.inheritance-info')[0];
    var infoHTML = '';
    ...
    if (settings.IsInherited) {
        disable();
        infoHTML = '<div class="icon"><span></span></div>';
        infoHTML += '<label>' + this.get_terminology()['InheritedField'];
        infoHTML += '<a href="/Admin/Content/Items/ItemTypes/ItemTypeEdit.aspx?SystemName=' + settings.Parent + '"><strong> ' + settings.Parent + '.</strong></a>'
        infoHTML += '<label>';

        inheritanceInfo.innerHTML = infoHTML;
    ...
    $('param-required').checked = !!settings.Required;
    ...
    $('param-excludefromsearch').checked = !!settings.IsExcludedFromSearch;
    ...
    if (parent && parent === this._systemName) {
        ...
        this.validateField((function () {
            ...
            if (this._currentSettingsTarget && settings) {
                ...
                settings.Required = $('param-required').checked;
                ...
                settings.IsExcludedFromSearch = $('param-excludefromsearch').checked;
                ...
                if ($("param-editor-configuration").style.display == "none") {
                    ...
                    var formHiddenElements = $$('#param-editor-configuration input[type=hidden]');
                    var serializedElements = Form.serializeElements(formElements.concat(formHiddenElements).concat(richControls), true)

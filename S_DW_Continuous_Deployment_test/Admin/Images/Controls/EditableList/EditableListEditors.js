...
Dynamicweb.Controls.EditableList.Editors.CheckboxEditor.prototype.format = function (value) {
    return value ? "<img src=\"/Admin/images/Check.gif\">" : "<img src=\"/Admin/images/Minus.gif\">";
};
...
Dynamicweb.Controls.EditableList.Editors.DateEditor.prototype = new Dynamicweb.Controls.EditableList.Editors.AjaxControlEditor();
...
Dynamicweb.Controls.EditableList.Editors.DateEditor.prototype.convert = function (str) {
    var result = str;

    if (typeof result === 'string') {
        result = new Date(result);
        if (!this._control._includeTime) {
            result = new Date(Date.UTC(result.getUTCFullYear(), result.getUTCMonth(), result.getUTCDate()))
        }
    }
...
Dynamicweb.Controls.EditableList.Editors.DateEditor.prototype.initialize = function (metadata) {
    if (!this._initialized) {
        ...
        var rootElement = this._control.get_container();

        this._control.get_calendar().cumulativeOffset = function (element) {
            var top = 0, left = 0, parent;
            ...
            return { top: top - element.height, left: left - element.width };
        };
    }
};
...
Dynamicweb.Controls.EditableList.Editors.NumberEditor.prototype = new Dynamicweb.Controls.EditableList.Editors.AjaxControlEditor();
...
Dynamicweb.Controls.EditableList.Editors.UserEditor.prototype.initialize = function (metadata) {
    if (!this._initialized) {
        var callback,
            sessionId = parseInt(Math.random().toString().substr(2), 10),
            handler = function (id, name, userCount) {
                if (callback) {
                    callback(id, name);
                }
            },
        ...
        if (!window[sessionId]) {
            window[sessionId] = handler;
        }

        this._control.set_provider(function (options) {
            var url,
                title,
                params = ['CompatibleWithOldExtranet=false'];
            ...
            if (metadata.select === 'User') {
                url = '/Admin/Module/UserManagement/SelectUserPopup.aspx';
                title = 'AddUserPopup';
                params.push('OnlyBackend=' + metadata.onlyBackend);
                params.push('HideAdmins=' + metadata.hideAdmins);
                params.push('UserCallBack=window[' + sessionId + ']');
            } else if (metadata.select === 'Group') {
                url = '/Admin/Module/UserManagement/SelectGroupPopup.aspx';
                title = 'AddGroupPopup';
                params.push('ShowSmartSearch=false');
                params.push('GroupCallBack=window[' + sessionId + ']');
            } else {
            ...
            if (url && params.length) {
                url += '?' + params.join('&');
                openWindow(url, (title || 'User managment'), 600, 400);
            }
        });
    }
};

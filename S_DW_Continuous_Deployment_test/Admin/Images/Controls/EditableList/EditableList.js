...
Dynamicweb.Controls.EditableList.Table = function () {
    ...
    _self.initialize = function (params) {
        ...
        if (!_initialized) {
            ...
            _delBtnColumn.innerHTML = '<span></span>';
            ...
            html += '<span>';
            html += (params.deleteColumnCaption || 'Delete');
            html += '</span>';
            html += '</div>';
            ...
            _addNewRow = _ajaxDoc.createElement('tr', {
                'class': 'editable-list-row editable-list-button add-button'
            });
            _addNewRowColumn = _ajaxDoc.createElement('td', {
                'colspan': _metadata.count() + 1 //edit button header
            });
            _addNewRowColumn.innerHTML = '<span> ' + (params.addNewRowCaption || 'Click here to add a new row') + '... </span>';
...
Dynamicweb.Controls.EditableList.Pager = function () {
    ...
    _self.initialize = function (params) {
        if (!_initialized) {
            ...
            _container = _ajaxDoc.createElement('div', { 'class': 'footer-controls editable-list-pager' });

            _ctlText = _ajaxDoc.createElement('span', { 'class': 'footer-label pager-label' });

            _btnToBegin = _ajaxDoc.createElement('img', {
                'align': 'absmiddle',
                'class': 'editable-list-button',
                'border': '0',
                'data-role': 'pager',
                'data-action': 'toBegin',
                'src': '/Admin/Images/Ribbon/UI/List/navigate_beginning.png'
            });

            _btnToPrev = _ajaxDoc.createElement('img', {
                'align': 'absmiddle',
                'class': 'editable-list-button',
                'border': '0',
                'data-role': 'pager',
            ...
            _btnToNext = _ajaxDoc.createElement('img', {
                'align': 'absmiddle',
                'class': 'editable-list-button',
                'border': '0',
                'data-role': 'pager',
                'data-action': 'toNext',
                'src': '/Admin/Images/Ribbon/UI/List/navigate_right.png'
            });

            _btnToEnd = _ajaxDoc.createElement('img', {
                'align': 'absmiddle',
                'class': 'editable-list-button',
                'border': '0',
                'data-role': 'pager',
                'data-action': 'toEnd',
                'src': '/Admin/Images/Ribbon/UI/List/navigate_end.png'
            });
            ...
            _self.set_isVisible(params.visible || true);
            _self.refresh();
...
Dynamicweb.Controls.EditableList.Footer = function () {
    ...
    _self.initialize = function (params) {
        if (!_initialized) {
            ...
            _pager.initialize({
                parent: _container,
                isVisible: (params.allowPaging || true)
            });
...
Dynamicweb.Controls.EditableList.Dialog = function () {
    ...
    function toggleEditor(editor, visible) {
        var row = _doc.parent(editor.get_container(), 'tr');
...
Dynamicweb.Controls.EditableList.Model = function (id, properties, state) {
    ...
    _self.serialize = function () {
        return {
            ...
            properties: _typeHelper.clone(_properties)
        };
    };
...
Dynamicweb.Controls.EditableList.ModelCollection = function () {
    var _self = this,
        ...
        _onModelPropertyChanged = function (eventName, args) {
            _eventEmitter.fire('property', { sender: _self, model: args.sender, name: args.name, value: args.value });
        };
...
Dynamicweb.Controls.EditableList.List = function () {
    ...
    function resize() {
        ...
        if (_self.get_allowPaging()) {
            ...
            if (height > 0) {
                ...
                dataCount = _self.count(_renderableStates) - ((currentPage - 1) * _pageSize);
                footerMargin = height - _heightInfo.footer - _heightInfo.tableAddNewRowButton - _heightInfo.tableHeaderRow - (_heightInfo.tableDataRow * (dataCount > _pageSize ? _pageSize : dataCount));
    ...
    _self.initialize = function () {
        if (!_self._initialized) {
            ...
            _table.initialize({ parent: this.get_container(), allowEditing: this.get_allowEditing(), allowSorting: this.get_allowSorting() });

...
Dynamicweb.Controls.CustomListBox.UnderlyingControl.prototype.reset = function () {
    if (this._initialized) {
        ...
        this._parent.removeChild(this._addButton);
        this._parent.removeChild(this._removeButton);
        this._parent.removeChild(this._clearButton);
        this._parent.removeChild(this._clearDiv);
...
Dynamicweb.Controls.CustomListBox.UnderlyingControl.prototype.addItem = function (item) {
    ...
    itemText = doc.createElement('span', { 'class': 'text' });
    itemText.innerHTML = item.text || item.value;

    itemIcon = doc.createElement('div', { 'class': 'icon' });

    if (!item.icon) {
        doc.addClass(itemIcon, 'no-icon');
    } else {
        img = doc.createElement('img');
        doc.attribute(img, 'src', item.icon);
...
Dynamicweb.Controls.CustomListBox.UnderlyingControl.prototype.initialize = function (parameters) {
    ...
    if (!this._initialized) {
        ...
        this._list = ajaxDoc.createElement('ul', {
            'class': 'std custom-listbox-list'
        });

        this._addButton = ajaxDoc.createElement('a', {
            'href': '#',
            'title': parameters.addTitle || 'Add',
            'class': 'std custom-listbox-btn custom-listbox-btn-add'
        });

        this._removeButton = ajaxDoc.createElement('a', {
            'href': '#',
            'title': parameters.removeTitle || 'Remove',
            'class': 'std custom-listbox-btn custom-listbox-btn-remove'
        });

        this._clearButton = ajaxDoc.createElement('a', {
            'href': '#',
            'title': parameters.clearTitle || 'Clear',
            'class': 'std custom-listbox-btn custom-listbox-btn-clear'
        });

        this._clearDiv = ajaxDoc.createElement('div', {
            'class': 'custom-selector-clear'
        });
        this._clearDiv.innerHTML = '&nbsp;';

        this._parent.appendChild(this._list);
        this._parent.appendChild(this._addButton);
        this._parent.appendChild(this._removeButton);
        this._parent.appendChild(this._clearButton);
        this._parent.appendChild(this._clearDiv);

        ajaxDoc.subscribe(this._list, 'click', 'li', funcHelper.proxy(function (event, element) {
            if (this.get_isEnabled()) {
                if (!ajaxDoc.hasClass('selected')) {
...
Dynamicweb.Controls.CustomListBox.ListBox.prototype.initialize = function () {
    ...
    form = ajaxDoc.getElementsBySelector('form')[0];

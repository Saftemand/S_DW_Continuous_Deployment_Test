...
function editStep(stepIndex) {
    ...
    var qRow = document.getElementById('StepQuoteRow');
    if (qRow){
        qRow.style.display = step.IsCheckout ? 'table-row' : 'none';
    }
...
function updateSteps() {
    ...
    for (i = 0; i < steps.length; i++) {
        ...
        row.insertCell(row.cells.length).appendChild(createImage('/Admin/Module/eCom_CartV2/images/notebook_edit.png', 'editStep(' + i + ');', 'Edit'));
        if (step.IsCheckout)
            row.insertCell(row.cells.length);
        else 
            row.insertCell(row.cells.length).appendChild(createImage('/Admin/Module/eCom_CartV2/images/notebook_delete.png', 'deleteStep(' + i + ');', 'Delete'));
    }

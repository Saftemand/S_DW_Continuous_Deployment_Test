<%@ Page CodeBehind="Calender_category_list.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="Dynamicweb.Admin.Calender_category_list" %>
						...
						<div ID="Tab1" STYLE="display:;width:598;">
							<table border="0" cellpadding="0" width="598">
								...
								<tr>
									...
									<td width="50" align="center"><strong><%=Translate.Translate("Begivenheder")%></strong></td>
...
Do While drCategory.Read()
		...
		Dim cnCount		As IDbConnection	= Database.CreateConnection("Dynamic.mdb")
		...
		If drCount.Read() Then
			...
			If Base.HasAccess("CalenderCategories", drCategory("CalenderCategoryID")) Then
				%>
									<tr>
										<td height="17" width="450" align="left">&nbsp;<a <%If Base.HasAccess("CalenderEdit", "") Then%> href="Calender_event_list.aspx?CalenderCategoryID=<%=drCategory("CalenderCategoryID").ToString%>&CalenderCategory=
										<%=Server.UrlEncode(drCategory("CalenderCategory").ToString())%>" <%			End If%>><%=Server.HtmlEncode(drCategory("CalenderCategory").ToString())%></a></td>
										<td width="50" align="center"><%=drCount("Amount").ToString%></td>
										<%If Base.HasAccess("CalenderDelete", "") Then%>
										<td width="48" align="center"><a href="javascript:DeleteCalenderCategory('<%=drCategory("CalenderCategoryID").ToString%>','<%=Base.JSEnable(Server.HtmlEncode(drCategory("CalenderCategory").ToString()))%>');"><img src="../../images/Delete.gif" border="0" alt="<%=Translate.JsTranslate("Slet %%", "%%", Translate.JSTranslate("kategori"))%>"</a></td>
										<%			Else%>

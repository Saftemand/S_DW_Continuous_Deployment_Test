...
Dynamicweb.Controls.OMC.ValidationList.prototype.saveAndClose = function (onComplete) {
    ...
    if(parent.ValidationEmailPopUp_wnd != null)
        parent.ValidationEmailPopUp_wnd.hide();
    else
        parent.pwDialog_wnd.hide();

    //if (parent.$('RecipientsExcluded')) {
    //    var excluded = parent.$('RecipientsExcluded').value;
    //    if (excluded.indexOf(',') != -1 || excluded.length != 0) {
    //        excluded += "," + recipientId;
    //    } else if (excluded.length == 0) {
    //        excluded = recipientId;
    //    }
    //    parent.$('RecipientsExcluded').value = excluded;
    //}

    //Dynamicweb.Ajax.doPostBack({
    //    eventTarget: self.get_uniqueID(),
    //    eventArgument: 'SaveAndClose:',
    //    onComplete: function (transport) {
    //        self._isBusy = false;
    //        parent.ValidationEmailPopUp_wnd.hide();

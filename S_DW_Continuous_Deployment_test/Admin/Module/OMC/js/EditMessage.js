...
OMC.MessageEdit.prototype.save = function (closeWindow, publish) {
    var self = this;
    Toolbar.setButtonIsActive('btnSaveNews', false);
    Toolbar.setButtonIsDisabled('btnSaveNews', true);
    /* Validating input */
    this.validateData(function (isValid) {
        ...
        Toolbar.setButtonIsDisabled('btnSaveNews', false);
    }, publish);
}
...
OMC.MessageEdit.prototype.scheduledPublish = function () {
    var self = this;
    Toolbar.setButtonIsDisabled('btnScheduledMessage', true);
    this.validateData(function (isValid) {
        if (isValid) {
            var form = self._getPostbackForm();
            if (form) {
        ...
        Toolbar.setButtonIsDisabled('btnScheduledMessage', false);
    }, true);
...
OMC.MessageEdit.prototype.updateToolbarBtns = function () {
    ...
    var buttons = Toolbar;
    if (buttons) {
        buttons.setButtonIsActive('btnSaveAndPublish', false);
        buttons.setButtonIsActive('btnScheduledMessage', false);
        $('btnSaveAndPublish').select('a.toolbar-button')[0].setAttribute('onclick', '');
        $('btnScheduledMessage').select('a.toolbar-button')[0].setAttribute('onclick', '');

        buttons.setButtonIsDisabled('btnSaveAndPublish', isDisabledPublish);
        buttons.setButtonIsDisabled('btnScheduledMessage', isDisabledPublish);

        if (!isDisabledPublish) {
            $('btnSaveAndPublish').select('a.toolbar-button')[0].setAttribute('onclick', this._clientInstanceName + '.saveAndPublish();');
            $('btnScheduledMessage').select('a.toolbar-button')[0].setAttribute('onclick', this._clientInstanceName + '.scheduledPublish();');
        }
    }
}
            ...
            if(response.Saved == true)
            {
                ...
                if(response.Close == true)
                {
                    ...
                    if (!response.Published && !response.Scheduled && response.FolderID && response.FolderID != -1)
                        ...
                        self.navigate('/Admin/Module/OMC/SMP/Statistics.aspx?messageId=' + response.ID);
                }
                else

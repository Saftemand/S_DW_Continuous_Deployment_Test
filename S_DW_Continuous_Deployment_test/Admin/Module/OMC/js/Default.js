...
OMC.MasterPage.prototype.autoSize = function () {
    ...
    accordionHeight = this.get_cache().accordion.getHeight();
    containerHeight = bodyHeight - this.get_cache().treeTitleOffset.top - accordionHeight + subTitleHeight;

    if (containerHeight > 0) {
        this.get_cache().tree.style.height = containerHeight + 'px';
        this.get_cache().accordionContainer.style.top = (containerHeight + this.get_cache().treeTitleOffset.top - subTitleHeight - 1) + 'px';
    }

    this.get_cache().frame.style.height = (bodyHeight - contentTitleHeight) + 'px';
}
...
OMC.MasterPage.prototype.toggleTreeCollapse = function () {
    ...
    this.get_cache().loadingIndicator.setStyle({ left: (this._treeCollapsed ? '25px' : '251px') });
...
{
    document.getElementById('ContentFrame').contentWindow.dialog.showAt('exportDialog', 100, 0);
}

OMC.MasterPage.prototype.LoadBounces = function (emailID, title) {
    title = title || 'Failures';
    if (emailID > 0) {
        this.showDialog('/Admin/Module/OMC/Emails/ValidateEmail.aspx?EmailId=' + emailID + '&Bounces=true&SaveAndClose=', 800, 420, { title: title , hideCancelButton: true });
    }
}
...
OMC.MasterPage.prototype.editTopFolderPermissions = function (topFolderID)
{
    ...
    {
        var id = topFolderID.split(":")[1];
        document.getElementById('permissionEditDialogIFrame').src = '/Admin/FastLoadRedirect.aspx?redirect=/Admin/Filemanager/Browser/FileManagerPermissionEdit.aspx?isEmailMarketing=true%26ID=1%26AccessElementType=emailfolder%26Element=' + id + '%26CloseOnExit=True%26DialogID=permissionEditDialog';
...
OMC.MasterPage.prototype.reloadNewsletters = function (newsletterID, nodeItemID) {
    var self = this;
    var params = {
        onComplete: function () {
...
OMC.MasterPage.prototype.showDialog = function (url, width, height, params, closedCallback)
{
    ...
    if (url && url.length)
    {
        ...
        if (d)
        {
            if (params.title) d.set_title(params.title);
            if (params.hideCancelButton) d.get_cancelButton().style.display = 'none';
            if (width > 0) d.set_width(width);
            if (height > 0) d.set_height(height);

            d.set_contentUrl(url);
            d._events.removeAllHandlers('hide');

            if (closedCallback != null)
                d.add_hide(closedCallback);
            else
                d._events.removeAllHandlers('hide');
...
OMC.MasterPage.prototype._setContentIsVisible = function (isVisible) {
    ...
    this.get_cache().loadingIndicator.setStyle({ display: (isVisible ? 'none' : ''), left: (this._treeCollapsed ? '25px' : '251px') });
...
OMC.MasterPage.onAccordionContextMenuView = function (sender, arg) {
    return arg.callingID;
}

OMC.MasterPage.onAccordionContextMenu = function (sender, arg) {

    ...
    <script type="text/javascript">
        function socialMessageSelected() {
            if (List && List.getSelectedRows('lstSocialMessages').length > 0) {
                parent.$('cmdDelete').select('a.toolbar-button')[0].setAttribute('onclick', 'OMC.MasterPage.get_current().confirmDeleteSocialMessage(<%=FolderId%>, <%=TopFolderId%>);');
                parent.Toolbar.setButtonIsDisabled('cmdDelete', false);
                parent.$('cmdMove').select('a.toolbar-button')[0].setAttribute('onclick', 'OMC.MasterPage.get_current().moveSocialMessage(null, <%=FolderId%> , <%=TopFolderId%>);');
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <dw:ContextMenu ID="menuEditMessage" OnShow="" runat="server">
        <dw:ContextMenuButton ID="cmdMoveSocialMessage" Text="Move message" ImagePath="/Admin/Images/Ribbon/Icons/Small/MoveDocument.png" OnClientClick="" runat="server" />
        <dw:ContextMenuButton ID="cmdDeleteSocialMessage" Text="Delete message" ImagePath="/Admin/Images/Ribbon/Icons/Small/Delete.png" OnClientClick="" runat="server" />
        <dw:ContextMenuButton ID="cmdEditSocialMessage" Text="Edit message" ImagePath="/Admin/Images/Ribbon/Icons/Small/Edit.png" OnClientClick="" runat="server" />
    </dw:ContextMenu>

    <dw:ContextMenu ID="menuPublishedMessage" runat="server">
        <dw:ContextMenuButton ID="cmdStatistics" Text="Statistics" ImagePath="/Admin/Images/Ribbon/Icons/Small/column-chart.png" OnClientClick="parent.OMC.MasterPage.get_current().set_contentUrl('/Admin/Module/OMC/SMP/Statistics.aspx?MessageID=' + encodeURIComponent(ContextMenu.callingItemID));" runat="server" />
    </dw:ContextMenu>
    <script type="text/javascript">

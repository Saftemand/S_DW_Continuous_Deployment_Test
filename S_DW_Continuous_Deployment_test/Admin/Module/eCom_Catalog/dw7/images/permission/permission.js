...
function addItem(type, id, evt) {
	...
	if(htRights["RIGHTS_" + id] == undefined){
		...
		var url = "/Admin/Module/eCom_Catalog/dw7/Edit/EcomPermission.aspx?CMD=GETINEFFECT&TYPE=" + RequestQueryString("CMD") + "&ID=" + RequestQueryString("ID") + "&GROUPID=" + id.toString();
...
function CreateListOfCurrentRights(){
	...
	for(var key in htRights)
	{
        ...
        if(dr.isGroup){
			img.src = '/admin/Module/eCom_Catalog/dw7/images/usergroup_folder.gif'
        }else{
			img.src = '/admin/Module/eCom_Catalog/dw7/images/user_user.gif'
        }
        img.align = 'middle';

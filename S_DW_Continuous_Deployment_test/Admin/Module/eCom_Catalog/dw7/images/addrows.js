...
var propGrpAdd = true;
var propFieldAdd = true;
var propOptionAdd = true;
var levelA = true;
var levelB = false;
var levelC = false;
...
try {
    ...
    var Stock6 = '<input class="NewUIinput" style="width:200px;" type="text" maxlength="255" id="STCKS_Icon#DWCNT#" name="STCKS_Icon#DWCNT#" value="" size="25"> <img src="/Admin/Images/Browse.gif" onClick="browseArchive(\'STCKS_Icon#DWCNT#\', \'\', document.all.STCKS_Icon#DWCNT#.value);" align=absMiddle class=H>';
} catch (e) {
    //Nothing
}
...
try {
    var Detail1 = '<input type=hidden id="DETAIL_IDTYPE[0]#DWCNT#" name="DETAIL_IDTYPE[0]#DWCNT#" value="0;#DWCNT#"><input type="hidden" id="DETAIL_ID[0]#DWCNT#" name="DETAIL_ID[0]#DWCNT#" value="-1"><input type=hidden id=DETAIL_LineID[0]#DWCNT# name=DETAIL_LineID[0]#DWCNT# value=#DWCNT#><input type=hidden id=DETAIL_Type[0]#DWCNT# name=DETAIL_Type[0]#DWCNT# value=0><input type=hidden id=Link_DETAIL_Value[0]#DWCNT# name=Link_DETAIL_Value[0]#DWCNT#><input type="text" class="NewUIinput" maxlength="255" name="DETAIL_Value[0]#DWCNT#" id="DETAIL_Value[0]#DWCNT#" value="" style="width:94%">&nbsp;<img src="/Admin/Images/Browse.gif" onClick=browseDetailArchive(0,#DWCNT#); align=absMiddle class=H>&nbsp;<img src=/Admin/Images/Icons/Page_int.gif onClick=internal(0,"","DETAIL_Value[0]#DWCNT#",""); align=absMiddle class=H>';
    var Detail2 = '<input type=hidden id="DETAIL_IDTYPE[1]#DWCNT#" name="DETAIL_IDTYPE[1]#DWCNT#" value="1;#DWCNT#"><input type="hidden" id="DETAIL_ID[1]#DWCNT#" name="DETAIL_ID[1]#DWCNT#" value="-1"><input type=hidden id=DETAIL_LineID[1]#DWCNT# name=DETAIL_LineID[1]#DWCNT# value=#DWCNT#><input type=hidden id=DETAIL_Type[1]#DWCNT# name=DETAIL_Type[1]#DWCNT# value=1><textarea id="DETAIL_Value[1]#DWCNT#" name="DETAIL_Value[1]#DWCNT#" cols="100" rows="10" style="width:100%"></textarea>';
} catch (e) {
    //Nothing
}

try {
    var PropGrp1 = '<input type="hidden" id="PROPGRP_ID#DWCNT#" name="PROPGRP_ID#DWCNT#" value="-1"><input type=hidden id=PROPGRP_LineID#DWCNT# name=PROPGRP_LineID#DWCNT# value=#DWCNT#><input type=hidden id=DETAIL_Type#DWCNT# name=DETAIL_Type#DWCNT# value=0><input type="text" class="NewUIinput" maxlength="255" id="PROPGRP_Name#DWCNT#" name="PROPGRP_Name#DWCNT#" value="" style="width:100%">';
    var PropFld1 = '<input type="hidden" id="PROPFLD_ID#DWCNT#" name="PROPFLD_ID#DWCNT#" value="-1"><input type=hidden id=PROPFLD_LineID#DWCNT# name=PROPFLD_LineID#DWCNT# value=#DWCNT#><input type=hidden id=DETAIL_Type#DWCNT# name=DETAIL_Type#DWCNT# value=0><input type="text" class="NewUIinput" maxlength="255"  id="PROPFLD_Label#DWCNT#"  name="PROPFLD_Label#DWCNT#" value="" style="width:100%">';
    var PropFld2 = PROPFLD_InputTypeValues;
    var PropOpt1 = '<input type="hidden" id="PROPOPTION_ID#DWCNT#" name="PROPOPTION_ID#DWCNT#" value="-1"><input type=hidden id=PROPOPTION_LineID#DWCNT# name=PROPOPTION_LineID#DWCNT# value=#DWCNT#><input type=hidden id=DETAIL_Type#DWCNT# name=DETAIL_Type#DWCNT# value=0><input type="text" maxlength="255" class="NewUIinput" id="PROPOPTION_Text#DWCNT#" name="PROPOPTION_Text#DWCNT#" value="" style="width:100%">';
} catch (e) {
    //Nothing
}
...
function AddDWRowLine(typeStr, prefix) {
    ...
    if (enableClick) {
        ...
        if (typeStr == "PROPGRP") {
            ...
            cellLength = oRow.cells.length;
            oCell = oRow.insertCell(cellLength);
            oCell.innerHTML = replaceSubstring(PropGrp1, NextFindIDStr, NextID);
        ...
        if (typeStr == "PROPFLD") {
            ...
            cellLength = oRow.cells.length;
            oCell = oRow.insertCell(cellLength);
            oCell.innerHTML = replaceSubstring(PropFld1, NextFindIDStr, NextID);

            cellLength = oRow.cells.length;
            oCell = oRow.insertCell(cellLength);
            oCell.innerHTML = replaceSubstring(PropFld2, NextFindIDStr, NextID);
        ...
        if (typeStr == "PROPOPT") {
            ...
            cellLength = oRow.cells.length;
            oCell = oRow.insertCell(cellLength);
            oCell.innerHTML = replaceSubstring(PropOpt1, NextFindIDStr, NextID);
...
function DeleteDWRow(DWRowid, rowID, typeStr, methodID, prefix, arg1, arg2, message) {
    ...
    if (deleteRow) {
        ...
        if (typeStr != "PRODPRICECALC" && typeStr != "PRODPROPREL" && typeStr != "PRODITEM" && typeStr != "VARIANT") {
            DeleteTR("DWRowLineTable", "DWRowLineTR", rowID, prefix, "");
        }
        ...
        if (typeStr == "PROPGRP") {
            FillDivLayer('EMPTY', '', 'FIELD');
            FillDivLayer('EMPTY', '', 'OPTION');
            EcomUpdator.document.location.href = "EcomUpdator.aspx?CMD=DeletePropertyGroup&ID=" + DWRowid + "&proptypeId=" + methodID + "&LangID=" + arg1;
        }
        if (typeStr == "PROPFLD") {
            FillDivLayer('EMPTY', '', 'OPTION');
            EcomUpdator.document.location.href = "EcomUpdator.aspx?CMD=DeletePropertyField&ID=" + DWRowid + "&proptypeId=" + methodID + "&LangID=" + arg1;
        }
        if (typeStr == "PROPOPT") {
            EcomUpdator.document.location.href = "EcomUpdator.aspx?CMD=DeletePropertyOption&ID=" + DWRowid + "&proptypeId=" + methodID + "&LangID=" + arg1;
        }
        if (typeStr == "PRODPROPREL") {
            FillDivLayer("LOADING", "", "PRODPROPREL");
            EcomUpdator.document.location.href = "EcomUpdator.aspx?CMD=DelProductPropertyRelation&ID=" + DWRowid + "&typeID=" + arg1;
        }
        if (typeStr == "PRODITEM") {
...
function AddDWRowFromArry(typeStr, masterID, contentArray, root, arg1, arg2) {
    ...
    if (typeStr == "GETPROPFIELD") {
        EcomUpdator.document.location.href = root + "EcomUpdator.aspx?CMD=GetPropertyFields&ID=" + masterID + "&GroupID=" + arg1 + "&LangID=" + arg2;
    }
    if (typeStr == "GETPROPOPTION") {
        EcomUpdator.document.location.href = root + "EcomUpdator.aspx?CMD=GetPropertyOptions&ID=" + masterID + "&FieldID=" + arg1 + "&LangID=" + arg2;
    }

    if (typeStr == "SETPRODPROPREL") {
        EcomUpdator.document.location.href = root + "EcomUpdator.aspx?CMD=SetProductPropertyRelation&ID=" + masterID + "&typeID=" + arg1;
    }
...
function enablePropertyButtons() {
    enablePropButton(levelA, "addPropGrpBut");
    enablePropButton(levelB, "addPropFieldBut");
    enablePropButton(levelC, "addProdOptionBut");
}

function enablePropButton(divName, buttonName) {
    if (divName == false) {
...
function setBooleanValue(ButName, value) {
    ...
    if (ButName == "addPropGrpBut") {
        propGrpAdd = value;
    }
    if (ButName == "addPropFieldBut") {
        propFieldAdd = value;
    }
    if (ButName == "addProdOptionBut") {
        propOptionAdd = value;
    }
